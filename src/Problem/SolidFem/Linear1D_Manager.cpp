...
void Linear1D::runNoInputExample() {
  ...
  MyUtils::Db::pr("line 15");
  for(int i=0; i<nodeX_0.size()-1; ++i)
    elements_.push_back(new Element::Line2(std::vector<int>{{i, i+1}}, Vectord{{nodeX_0(i), nodeX_0(i+1)}}));
...
Matrix2d Linear1D::assembleK() {
  ...
  for(int e=0; e<elements_.size(); ++e) {
    ...
    for(int locId=0; locId<eleGlobalNodeIds.size(); ++locId) {
      MyUtils::Db::pr("line54: "+std::to_string(eleGlobalNodeIds[locId]+1));
  ...
  MyUtils::Db::pr("line44");
  MyUtils::Db::pr(std::to_string(globalNodeCount));
  for(int e=0; e<elements_.size(); ++e) {
