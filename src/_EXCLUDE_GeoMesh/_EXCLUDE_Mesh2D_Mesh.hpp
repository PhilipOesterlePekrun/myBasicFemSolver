...
class Mesh {
 public:
  Mesh();
 protected:
  std::vector<Vectord> nodeCoords_; // Array of (x, y) coords for global node Ids
  std::vector<std::vector<int>> elements_; // Array of elements, each consisting of a set of some global node IDs (e.g. three nodes for Tri3), with CCW node order
  std::vector<int> boundaryNodes_; // These are only the boundary nodes of the mesh; useful for applying BCs to e.g. all nodes with x_coord > 2.15, but without doing it on the inner nodes. This set is generally not equivalent to the given polygon (different IDs, nodes between polygon vertices)
};
